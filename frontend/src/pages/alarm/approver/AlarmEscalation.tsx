...
export default function AlarmEscalation({ adminName: _adminName, onNavigate }: Props) {
  ...
  function getRecipients(tier: 1 | 2 | 3): string {
    if (tier === 1) return 'Tester'
    if (tier === 2) return 'Tester + Approver'
    return 'Tester + Approver + Regional Controller'
  }

  function handleSendReminder(row: EscalationRow) {
    const recipients = getRecipients(row.tier)
    const confirmed = window.confirm(
      `Send ${TIER_STYLES[row.tier].label} escalation email for "${row.buildingName}"?\n\nRecipients: ${recipients}`
    )
    if (!confirmed) return
