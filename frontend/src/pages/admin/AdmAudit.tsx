...
export default function AdmAudit({ adminName }: Props) {
  ...
  const allLocations = apiLocations.length > 0 ? apiLocations : LOCATIONS.map(l => ({ id: l.id, name: l.name, cost_center: (l as unknown as { costCenter?: string; cost_center?: string }).costCenter || (l as unknown as { costCenter?: string; cost_center?: string }).cost_center }))
