...
import type { SectionTrends, ApiLocation } from '../../api/types'
...
const SECTIONS = [
  ...
  { key: 'secJ', label: 'Coin Purchase in Transit',      short: 'J', color: '#0e7490', base: 85   },
  { key: 'secK', label: "Total Cashier's Fund – TODAY",  short: 'K', color: '#15803d', base: 9575 },
]
...
// ── CSV export ─────────────────────────────────────────────────────────────
function downloadCSV(granularity: Granularity, periodN: number, locations: ApiLocation[]) {
  const allLocs = [
...
export default function RcTrends({ adminName }: Props) {
  ...
  const chartData = useMemo((): DataPoint[] => {
    if (apiTrends && apiTrends.data.length > 0) {
      return apiTrends.data.map(p => ({ period: p.period, [sectionKey]: p.avg_total }))
    }
    return mockData
  }, [apiTrends, mockData, sectionKey])

  const values    = chartData.map(r => (r[sectionKey] as number | string | undefined) ?? 0).map(Number)
  const latest    = apiTrends ? apiTrends.summary.latest_value   : (values.at(-1) ?? 0)
  const prev      = apiTrends ? apiTrends.summary.previous_value : (values.at(-2) ?? latest)
  const avg       = apiTrends ? Math.round(apiTrends.summary.period_avg) : (values.length ? Math.round(values.reduce((a, b) => a + b, 0) / values.length) : 0)
  const peak      = apiTrends ? apiTrends.summary.peak           : (values.length ? Math.max(...values) : 0)
  const total     = values.reduce((a, b) => a + b, 0)
  const pctChange = apiTrends ? apiTrends.summary.change_pct     : (prev ? ((latest - prev) / prev) * 100 : 0)
  const changeUp  = pctChange >= 0

  // daily maps to weekly on the API — show 'wks' when real API data is present
  const periodUnit = (granularity === 'daily' && apiTrends) ? 'wks'
    : granularity === 'daily' ? 'days'
    : granularity === 'weekly' ? 'wks'
    : granularity === 'monthly' ? 'mo' : 'qtrs'
      ...
      <div className="ph" style={{ marginBottom: 18 }}>
        ...
        <div className="ph-right">
          <button
            ...
            onClick={() => downloadCSV(granularity, effectivePeriodN, apiLocations)}
          >
            ↓ Download CSV
          </button>
        </div>
        ...
        <div className="card-body" style={{ padding: '16px 20px', display: 'flex', gap: 28, flexWrap: 'wrap', alignItems: 'flex-start' }}>
          ...
          <div style={{ flex: 1, minWidth: 260 }}>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              ...
              {apiLocations.filter(l => l.active).map(l => (
                <button
                  key={l.id}
      ...
      <div className="card">
        <div className="card-header">
          <div>
            ...
            {granularity === 'daily' && apiTrends && (
              <span style={{ marginLeft: 8, fontSize: 11, color: 'var(--amb)', fontWeight: 600 }}>
                (weekly aggregation — daily not supported by API)
              </span>
            )}
          </div>
          <div style={{
        ...
        <div className="card-body" style={{ padding: '20px 12px 12px' }}>
          <ResponsiveContainer width="100%" height={380}>
            <AreaChart data={chartData} margin={{ top: 8, right: 24, left: 8, bottom: 4 }}>
              ...
              />
              <Tooltip
                formatter={(value) => [formatCurrency(Number(value) || 0), activeSec.label]}
                contentStyle={{
