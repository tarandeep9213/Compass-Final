...
export default function AdmConfig({ adminName }: Props) {
  ...
  // Load config and locations from API on mount
  useEffect(() => {
  ...
  function setG(k: keyof typeof GLOBALS_DEFAULT, v: string) {
    setGlobals(p => ({ ...p, [k]: v })); setErrors(p => ({ ...p, [k]: '' }))
  }
  function setLoc(id: string, v: string) {
    setLocOverrides(p => ({ ...p, [id]: { tolerancePct: v } }))
  }
  function clearOverride(id: string) {
    setLocOverrides(p => { const n = { ...p }; delete n[id]; return n })
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      await Promise.all(apiLocations.map(loc => {
    ...
    <div className="fade-up">
      <div className="ph" style={{ marginBottom: 24 }}>
        ...
        <div className="ph-right">
          <button className="btn btn-primary" onClick={handleSave} disabled={isSaving}
            ...
            {isSaving && <Spinner size="sm" color="#fff" />}
      ...
      <div className="card" style={{ marginBottom: 22 }}>
        ...
        <div className="card-body" style={{ maxWidth: 700 }}>
          {configLoading && <SkeletonTable rows={3} />}
          {!configLoading && (<>
      ...
      <div className="card">
        ...
        <div className="card-body" style={{ padding: 0 }}>
          {configLoading && <SkeletonTable rows={3} />}
          {!configLoading && apiLocations.length === 0 && (
            <EmptyState icon="⚙️" title="No location overrides" subtitle="All locations use the system default settings." />
          )}
          {!configLoading && apiLocations.length > 0 && (
          <table className="dt">
            <thead>
