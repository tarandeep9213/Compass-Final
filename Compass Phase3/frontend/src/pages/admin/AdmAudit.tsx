...
import { AUDIT_EVENTS, todayStr } from '../../mock/data'
import type { AuditEvent } from '../../mock/data'
import { listAuditEvents } from '../../api/audit'
import { listLocations } from '../../api/locations'
import type { ApiAuditEvent, ApiLocation } from '../../api/types'
...
export default function AdmAudit({ adminName }: Props) {
  ...
  const availableLocations = useMemo(() => {
    ...
    return apiLocations.filter(l => locIds.has(l.id))
  }, [filterType, filterActor, sourceEvents, apiLocations])
  ...
  const getExportData = () => filtered.map(ev => {
    const loc = apiLocations.find(l => l.id === ev.locationId)
