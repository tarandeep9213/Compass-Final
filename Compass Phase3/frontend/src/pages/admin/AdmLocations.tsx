...
const EMPTY_FORM = { id: '', name:'', costCenter:'', expectedCash:'', tolerancePct:'' }
const PAGE_SIZE  = 10
...
export default function AdmLocations({ adminName }: Props) {
  ...
  const [isDefSaving, setIsDefSaving] = useState(false)
  const [page,     setPage]     = useState(0)
  const [defaults, setDefaults] = useState<{ tolerancePct: string; slaHours: string }>(DEFAULTS_INIT)
  ...
  function openEdit(loc: Location) {
    ...
    setMode({id:loc.id})
    setForm({ id: loc.id, name:loc.name, costCenter:loc.cost_center||'', expectedCash:String(loc.expectedCash), tolerancePct:String(loc.effectiveTolerancePct ?? loc.tolerancePct) })
    setErrors({})
    setConfirm(null)
  }
  ...
  function validate() {
    ...
    const editId = mode && typeof mode === 'object' ? mode.id : null
    ...
    if (form.name.trim() && locs.some(l => l.name.trim().toLowerCase() === nameLower && l.id !== editId))
      e.name = 'A location with this name already exists'

    const cash = Number(form.expectedCash)
    if (mode === 'add' && (!form.expectedCash || isNaN(cash) || cash <= 0)) e.expectedCash = 'Enter a valid amount'
    if (mode !== 'add' && form.expectedCash !== '' && (isNaN(cash) || cash < 0)) e.expectedCash = 'Enter a valid amount'
  ...
  function syncToStorage(_: Location[]) {
    // no-op: state is authoritative; data comes from API on next mount
  }
      ...
      if (mode === 'add') {
        ...
        try {
          ...
          if (err instanceof ApiError && err.status === 409) {
            setErrors({ name: err.message || 'A location with this name already exists' })
            return
          }
          // demo mode — backend unavailable, continue with local state
        }
        setLocs(prev => {
        ...
        try {
          const updated = await updateLocation(mode.id, { name: form.name.trim(), cost_center: form.costCenter.trim() || undefined, expected_cash: form.expectedCash !== '' ? Number(form.expectedCash) : undefined, tolerance_pct: Number(form.tolerancePct) } as Parameters<typeof updateLocation>[1])
          setLocs(prev => {
            const next = prev.map(l => l.id === (mode as {id:string}).id ? mapApiLocation(updated) : l)
            syncToStorage(next)
            return next
          })
      ...
      <div className="card">
        ...
        <div className="card-body" style={{padding:0}}>
          ...
          {!loading && locs.length === 0 && (
            <EmptyState icon="📍" title="No locations found" subtitle="Add a location to get started." />
          )}
          {!loading && locs.length > 0 && (<>
          <table className="dt">
            <thead>
                ...
                <>
                  <tr style={{background:'var(--g0)'}}>
                    ...
                    <td style={{textAlign:'right'}}>
                      <button className="btn btn-primary" style={{fontSize:12,padding:'5px 14px',display:'inline-flex',alignItems:'center',gap:5}} onClick={handleSave} disabled={isSaving}>
                        {isSaving && <Spinner size="sm" color="#fff" />}{isSaving ? 'Saving…' : 'Save'}
                      </button>
                      <button className="btn btn-ghost" style={{fontSize:12,marginLeft:6}} onClick={cancel} disabled={isSaving}>Cancel</button>
                    </td>
                  </tr>
                </>
              )}
                ...
                return (
                  <React.Fragment key={loc.id}>
                    <tr style={{opacity:loc.active?1:0.6}}>
                      ...
                      <td style={{textAlign:'center',fontSize:13}}>{loc.effectiveTolerancePct ?? loc.tolerancePct}%</td>
                      <td style={{textAlign:'center'}}>
                        <StatusBadge status={loc.active ? 'active' : 'inactive'} />
                      </td>
                      <td style={{textAlign:'right'}}>
                        ...
                        <td colSpan={6}>
                          <div style={{display:'flex',gap:14,flexWrap:'wrap',padding:'12px 4px',alignItems:'flex-end'}}>
                            <div style={{flex:'0 0 130px'}}>
                              ...
                              <input className="f-inp" value={form.costCenter} inputMode="numeric" pattern="[0-9]*" placeholder="e.g. 12345"
                                onChange={e=>{const v=e.target.value.replace(/\D/g,'');setForm(p=>({...p,costCenter:v}));setErrors(p=>({...p,costCenter:''}))}}
                                style={{width:'100%',fontSize:13}}/>
                              {errors.costCenter&&<div style={{fontSize:11,color:'var(--red)'}}>{errors.costCenter}</div>}
                            </div>
                            ...
                            <div style={{display:'flex',gap:8}}>
                              <button className="btn btn-primary" style={{fontSize:12,padding:'7px 18px',display:'inline-flex',alignItems:'center',gap:5}} onClick={handleSave} disabled={isSaving}>
                                {isSaving && <Spinner size="sm" color="#fff" />}{isSaving ? 'Saving…' : 'Save'}
                              </button>
                              <button className="btn btn-ghost" style={{fontSize:12}} onClick={cancel} disabled={isSaving}>Cancel</button>
                            </div>
                          </div>
                        </td>
                      </tr>
          ...
          <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap', alignItems: 'flex-start' }}>
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', alignSelf: 'center', marginTop: 14 }}>
              <button
                ...
                style={{ fontSize: 12, padding: '7px 18px', display: 'inline-flex', alignItems: 'center', gap: 5 }}
                onClick={handleSaveDefaults}
                disabled={isDefSaving}
              >
                {isDefSaving && <Spinner size="sm" color="#fff" />}
                {isDefSaving ? 'Saving…' : 'Save Defaults'}
              </button>
            </div>
          </div>
        </div>
