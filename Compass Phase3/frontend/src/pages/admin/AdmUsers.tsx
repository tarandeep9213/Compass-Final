...
export default function AdmUsers({ adminName }: Props) {
  ...
  async function saveGrant(u: User) {
    ...
    toast.success(`${label} access granted to ${u.name}.`)
  }
  ...
  async function revokeGrant(u: User, type: AccessType) {
    ...
    setGrants(type, updated); writeGrants(updated, type)
    const label = type === 'operator' ? 'Operator' : 'Controller'
    toast.success(`${label} access revoked for ${u.name}.`)
  }
  ...
  async function handleSave() {
    ...
    try {
      if (mode==='add') {
        ...
        toast.success(`User "${nu.name}" added.`)
  ...
  async function handlePurge() {
    ...
    try {
      ...
      toast.success(`Removed ${res.deleted} user${res.deleted !== 1 ? 's' : ''}. Admin account preserved.`)
    } catch {
                ...
                <tr style={{background:'var(--g0)'}}>
                  <td colSpan={6}>
                    <AddEditForm form={form} setForm={setForm} errors={errors} setErrors={setErrors}
                      locNeeded={locNeeded} toggleLoc={toggleLoc} locations={apiLocations} onSave={handleSave} onCancel={cancel} isAdd={true} isSaving={isSaving} />
                  </td>
                </tr>
              )}
                  ...
                  <React.Fragment key={u.id}>
                    ...
                    {isEditing && (
                      <tr key={u.id+'-edit'} style={{background:'var(--g0)'}}>
                        <td colSpan={6}>
                          <AddEditForm form={form} setForm={setForm} errors={errors} setErrors={setErrors}
                            locNeeded={locNeeded} toggleLoc={toggleLoc} locations={apiLocations} onSave={handleSave} onCancel={cancel} isAdd={false} isSaving={isSaving} />
                        </td>
                      </tr>
                    )}
        ...
        <div className="card-body">
          <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap', alignItems: 'flex-end' }}>
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              ...
              >
                {sysSaving && <Spinner size="sm" color="#fff" />}
...
function AddEditForm({ form, setForm, errors, setErrors, locNeeded, toggleLoc, locations, onSave, onCancel, isAdd, isSaving }:
  { form: UserForm; setForm: React.Dispatch<React.SetStateAction<UserForm>>; errors: FormErrors; setErrors: React.Dispatch<React.SetStateAction<FormErrors>>; locNeeded: boolean; toggleLoc:(id:string)=>void; locations: {id:string;name:string}[]; onSave:()=>void; onCancel:()=>void; isAdd: boolean; isSaving: boolean }) {
  ...
  return (
    <div style={{display:'flex',gap:14,flexWrap:'wrap',padding:'12px 4px',alignItems:'flex-start'}}>
        ...
        <div style={{flex:'1 1 220px'}}>
          ...
          <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
            {form.role === 'regional-controller' && (
              <label style={{display:'flex',alignItems:'center',gap:5,fontSize:11,cursor:'pointer',padding:'4px 8px',borderRadius:6,border:`1px solid ${form.locationIds.length === locations.length && locations.length > 0?'var(--g3)':'var(--ow2)'}`,background:form.locationIds.length === locations.length && locations.length > 0?'var(--g0)':'#fff',color:form.locationIds.length === locations.length && locations.length > 0?'var(--g7)':'var(--ts)',fontWeight:600}}>
                <input type="checkbox"
                  checked={form.locationIds.length === locations.length && locations.length > 0}
                  onChange={()=>{
                    const allIds = locations.map(l=>l.id)
                    const isAll = form.locationIds.length === locations.length
                    setForm(p=>({...p, locationIds: isAll ? [] : allIds}))
      ...
      <div style={{display:'flex',gap:8,alignSelf:'flex-end',paddingBottom:2}}>
        <button className="btn btn-primary" style={{fontSize:12,padding:'7px 18px',display:'inline-flex',alignItems:'center',gap:5}} onClick={onSave} disabled={isSaving}>
          {isSaving && <Spinner size="sm" color="#fff" />}{isSaving ? 'Saving…' : 'Save'}
        </button>
        <button className="btn btn-ghost" style={{fontSize:12}} onClick={onCancel} disabled={isSaving}>Cancel</button>
      </div>
    </div>
  )
}
