...
import { VERIFICATIONS, SUBMISSIONS, formatCurrency, IMPREST } from '../../mock/data'
...
export default function CtrlDashboard({ controllerName, locationIds, ctx, onNavigate }: Props) {
  ...
  // Map of locationId_date to { status, id, totalCash, expectedCash }
    ...
    if (locationIds.length > 0) {
      Promise.all(locationIds.map(id => listSubmissions({ location_id: id, page_size: 100 }).then(r => r.items)))
        .then(arrays => {
          ...
          flats.forEach(s => {
            map[`${s.location_id}_${s.submission_date}`] = { status: s.status, id: s.id, totalCash: s.total_cash, expectedCash: s.expected_cash }
          })
          setApiSubsMap(map)
        })
        .catch(() => { /* fall back to mock */ })
  ...
  */
  ...
  async function handleComplete(id: string) {
    ...
    // Use approved submission's total_cash as observed_total
        ...
        <div>
          ...
          <p style={{ color: 'var(--ts)', fontSize: 13 }}>
            ...
            {locationIds.length === 1
              ? ` · ${apiLocations.find(l => l.id === locationIds[0])?.name ?? locationIds[0]}`
              : ` · ${controllerName}`
            }
          </p>
        </div>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        >
          ...
          {locationIds.map(id => {
            ...
            return <option key={id} value={id} style={{ color: '#1f2937', backgroundColor: '#ffffff' }}>{loc?.name ?? id}</option>
          })}
        </select>
      ...
      <div className="card">
        <div className="card-header">
          ...
          <span className="card-sub">
            ...
            {locationFilter !== 'all' ? ` · ${apiLocations.find(l => l.id === locationFilter)?.name ?? locationFilter}` : ''}
        ...
        <div className="card-body" style={{ padding: 0 }}>
          {rows.length === 0 ? (
            ...
            <table className="dt" style={{ tableLayout: 'fixed', width: '100%' }}>
              ...
              <tbody>
                {pageRows.map(v => {
                  const loc        = apiLocations.find(l => l.id === v.locationId)
                  const sub        = apiSubsMap[`${v.locationId}_${v.date}`]
                  const isFuture   = v.date > todayStr
                  const isExpanded = expandedId === v.id
                  const imprest    = (loc?.expected_cash && loc.expected_cash > 0) ? loc.expected_cash : (sub?.expectedCash && sub.expectedCash > 0 ? sub.expectedCash : IMPREST)
                          ...
                          <td colSpan={8} style={{ padding: 0, borderBottom: '1px solid var(--ow2)' }}>
                            <div style={{
                              ...
                              <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', alignItems: 'stretch' }}>
                                ...
                                {/* Submission total — read-only, sourced from approved form */}
                                {(() => {
                                  const subTotal = apiSubsMap[`${v.locationId}_${v.date}`]?.totalCash
                                ...
                                {/* Observed total (commented out — now sourced from submission)
