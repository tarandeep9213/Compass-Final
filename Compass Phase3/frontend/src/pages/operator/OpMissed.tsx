...
import { getControllerForLocation } from '../../api/admin'
...
export default function OpMissed({ ctx, onNavigate }: Props) {
  ...
  // Auto-populate supervisor from the location's controller (API first, mock fallback)
  const mockController = USERS.find(u => u.role === 'controller' && u.locationIds.includes(ctx.locationId))

  const [reason, setReason] = useState('')
  const [detail, setDetail] = useState('')
  const [supervisorName, setSupervisorName] = useState(mockController?.name ?? '')

  useEffect(() => {
    getControllerForLocation(ctx.locationId)
      .then(c => { if (c.name) setSupervisorName(c.name) })
