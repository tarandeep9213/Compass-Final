...
export default function OpReadonly({ ctx, onNavigate }: Props) {
  ...
  const [coinTransitAmt, setCoinTransitAmt] = useState<number>(0)
  ...
  useEffect(() => {
    ...
    getSubmission(ctx.submissionId)
      .then(s => {
        ...
        const ct = typeof s.sections['coin_transit'] === 'number' ? (s.sections['coin_transit'] as number) : 0
        if (ct) setCoinTransitAmt(ct)
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 16 }}>
            <tbody>
              ...
              <tr style={{ background: '#fffde7', borderBottom: '2px solid var(--ow2)' }}>
                ...
                <td style={{ ...YC, padding: '8px 0', fontSize: 15, fontFamily: 'DM Serif Display,serif' }}>
                  {formatCurrency(sub.totalCash + coinTransitAmt)}
                </td>
              </tr>
            </tbody>
          </table>
