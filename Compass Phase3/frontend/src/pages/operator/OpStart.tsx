...
// Local draft shape used only for the "today draft" card
interface TodayDraftInfo { id: string; totalSoFar: number }
...
export default function OpStart({ locationIds, userName, onNavigate }: Props) {
  ...
  const [apiDrafts, setApiDrafts] = useState<ApiSubmission[]>([])
  ...
  function fetchSubs() {
    ...
    listSubmissions({ location_id: locationId, status: 'draft', page_size: 50 })
      .then(r => setApiDrafts(r.items))
      .catch(() => { /* keep existing apiDrafts */ })
  }

  useEffect(() => {
    fetchSubs()
    // Poll every 60 s so status changes (e.g. rejection) appear without manual navigation
    const timer = setInterval(fetchSubs, 60_000)
    // Also refresh when the browser tab becomes active again
    function onVisible() { if (document.visibilityState === 'visible') fetchSubs() }
  ...
  // Prefer API drafts (persisted); fall back to in-memory mock if API returned nothing
  const todayDraft: TodayDraftInfo | null = (() => {
    const apiD = apiDrafts.find(d => d.submission_date === todayStr())
    if (apiD) return { id: apiD.id, totalSoFar: apiD.total_cash }
    if (apiDrafts.length === 0) {
      const m = DRAFTS.find(d => d.locationId === locationId && d.date === todayStr())
      return m ? { id: m.id, totalSoFar: m.totalSoFar } : null
    }
    return null
  })()
  ...
  const historyRows: HistoryRow[] = useMemo(() => {
    // Include drafts when computing firstSubDate so history doesn't vanish if
    // all submissions temporarily become drafts (e.g. after editing a rejected form)
    const locationSubs = sourceSubs.filter(s => s.locationId === locationId)
  ...
  const draftCount = apiDrafts.length > 0 ? apiDrafts.length : DRAFTS.length
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, flexWrap: 'wrap' }}>
              <div>
                ...
                <div style={{ fontSize: 13, color: 'var(--ts)' }}>
                  ...
                  &nbsp;·&nbsp;Imprest: {formatCurrency(location?.expected_cash && location.expected_cash > 0 ? location.expected_cash : IMPREST)}
                </div>
