...
export default function OpMethod({ ctx, onNavigate }: Props) {
  ...
  // Fetch real expected_cash from API, fall back to mock IMPREST
  const [imprest, setImprest] = useState(location?.expectedCash ?? IMPREST)
  useEffect(() => {
    listLocations().then(locs => {
      const loc = locs.find(l => l.id === ctx.locationId)
      if (loc && loc.expected_cash > 0) setImprest(loc.expected_cash)
    }).catch(() => { /* keep fallback */ })
  }, [ctx.locationId])
