    ...
    try {
      ...
      if (raw) {
        ...
        return {
          ...
          coin_transit: typeof d.coin_transit === 'number' ? d.coin_transit : undefined,
          holdover: typeof d.holdover === 'number' ? d.holdover : undefined,
        }
      }
    } catch {
      /* ignore JSON parse error */
...
export default function OpForm({ ctx, onNavigate }: Props) {
  ...
  const [holdover,   setHoldover]   = useState(() => { const pf = getExcelPrefill(ctx); return pf?.holdover    ? String(pf.holdover)    : '' })
  const [coinTransit,setCoinTransit]= useState(() => { const pf = getExcelPrefill(ctx); return pf?.coin_transit ? String(pf.coin_transit) : '' })
  ...
  useEffect(() => {
    ...
    getSubmission(ctx.submissionId).then(sub => {
      ...
      const a = s.A ?? {}
      // If denomination detail exists, use it; otherwise put the total into "other" so the form isn't empty
      const hasDetail = a.ones || a.twos || a.fives || a.tens || a.twenties || a.fifties || a.hundreds || a.other
      setAQty({
      ...
      else if (!hasDetail && a.total) setAOther(String(a.total))

      const b = s.B ?? {}
      const bHasDetail = b.dollar || b.halves || b.quarters || b.dimes || b.nickels || b.pennies
      setBQty({
        bDollar:   b.dollar   ? String(b.dollar)   : (!bHasDetail && b.total ? String(b.total) : ''),
      ...
      if (set1.length || set2.length) {
        ...
        // No detail — put total as qty=1, amount=total in first row
        const rows = emptyQA5(); rows[0] = { qty: '1', amount: String(s.E.total) }
        setELeft(rows); setERight(emptyQA5())
      }

      const fRaw = (s.F?.rows ?? []) as { qty: unknown; amount: unknown }[]
      if (fRaw.length) {
        setFRows(fRaw.map(r => ({ qty: String(r.qty), amount: String(r.amount) })).concat(emptyQA4()).slice(0, 4))
      } else if (s.F?.total) {
        const rows = emptyQA4(); rows[0] = { qty: '1', amount: String(s.F.total) }
        setFRows(rows)
      }

      const g = s.G ?? {}
      if (g.currency) setGCurr(String(g.currency))
      else if (!g.currency && !g.coin && g.total) setGCurr(String(g.total))
      if (g.coin) setGCoin(String(g.coin))

      const h = s.H ?? {}
      if (h.value) setHVal(String(h.value))
      else if (h.total) setHVal(String(h.total))

      const i = s.I ?? {}
      if (i.yesterday) setIYest(String(i.yesterday))
      if (i.today) setIToday(String(i.today))
      else if (!i.yesterday && !i.today && i.total) setIToday(String(i.total))
  ...
  // Fetch real expected_cash and tolerance from API
  const [apiExpectedCash, setApiExpectedCash] = useState<number | null>(null)
  const [apiTolerance, setApiTolerance] = useState<number | null>(null)
  useEffect(() => {
    listLocations().then(locs => {
      const loc = locs.find(l => l.id === ctx.locationId)
      if (loc) {
        if (loc.expected_cash > 0) setApiExpectedCash(loc.expected_cash)
        if (loc.tolerance_pct != null) setApiTolerance(loc.tolerance_pct)
      }
    }).catch(() => { /* keep mock fallback */ })
  }, [ctx.locationId])

  const expectedCash = apiExpectedCash ?? location?.expectedCash ?? IMPREST
  const tolerance    = apiTolerance ?? location?.tolerancePct ?? 5
