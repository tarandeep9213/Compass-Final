...
// ── Controller lookup (available to any authenticated user) ───────────────────

export function getControllerForLocation(locationId: string): Promise<{ name: string; email: string }> {
  return api.get<{ name: string; email: string }>(`/users/controller-for-location?location_id=${encodeURIComponent(locationId)}`)
}
