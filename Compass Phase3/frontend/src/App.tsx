...
function navItems(role: Role): { id: string; icon: string; label: string; panel: string }[] {
  switch (role) {
    ...
    case 'admin': return [
      { id: 'audit',     icon: '📑', label: 'Cash Audit Trail',  panel: 'adm-audit'    },
...
function AppShell({ auth, onLogout }: { auth: AuthState; onLogout: () => void }) {
  ...
  const items = [
    ...
    ...(alarmTester && !baseItems.some(i => i.id === 'alarm-testing')
      ? [{ id: 'alarm-testing-grant', icon: '🔔', label: 'Alarm Testing', panel: 'alarm-history' }] : []),
  ...
  function renderPanel() {
    ...
    switch (panel) {
      ...
      // ── Regional Controller extra panels ─────────────────────────────
      ...
      case 'biannual-check':  return <BiannualCheck userName={auth.name} locationIds={auth.locationIds} ctx={ctx} onNavigate={navigate} />
