...
5. **Regional Controllers** provide senior oversight across all locations
6. **Admins** configure the system, manage users and view all compliance data
7. **Auditors** review reports and the full audit trail
...
| **DGM** | Schedules monthly oversight visits. Views monthly compliance dashboard and visit history. Can be granted Operator or Controller screen access by Admin. | Visit Dashboard, Visit History, Log Visit |
| **Admin** | Manages locations, users, and roster import. Grants/revokes screen access to DGM and Regional Controller users. | Locations, Users, Import Roster |
| **Regional Controller** | Senior oversight role. Views compliance dashboard, audit trail, reports, and cash count trends. Can be granted Operator or Controller screen access by Admin. | Compliance Dashboard, Audit Trail, Reports, Cash Trends |
...
| Regional Controller | regionalcontroller@compass.com | demo1234 |
...
├── utils/
│   └── operatorAccess.ts           ← localStorage helper for screen access grants
├── pages/
...
│   │   └── OpDrafts.tsx            ← Saved drafts list
│   ├── manager/
│   │   ├── MgrApprovals.tsx        ← Pending approvals queue
│   │   └── MgrHistory.tsx          ← Past approval decisions
│   ├── controller/
│   │   ├── CtrlLog.tsx             ← Schedule controller visit
│   │   ├── CtrlDashboard.tsx       ← Controller visit dashboard
│   │   └── CtrlHistory.tsx         ← Controller verification history
│   ├── dgm/
│   │   ├── DGMDash.tsx             ← Visit schedule dashboard (controller-style)
│   │   ├── DGMHistory.tsx          ← Full visit history with filters
│   │   └── DGMLog.tsx              ← Log DGM visit
│   ├── admin/
│   │   ├── AdmCompliance.tsx       ← Unified compliance dashboard
│   │   ├── AdmLocations.tsx        ← Manage locations + Global Defaults card
│   │   ├── AdmUsers.tsx            ← Manage users + System Settings + Screen Access Delegation
│   │   ├── AdmAudit.tsx            ← Audit trail
│   │   ├── AdmReports.tsx          ← Weekly/monthly reports
│   │   └── AdmImport.tsx           ← Roster Excel import
│   └── regional-controller/
│       └── RcTrends.tsx            ← Cash count section trends (area chart)
...
**Important:** The frontend currently shows quick-fill buttons for demo accounts. In production, remove these or gate them behind an env flag.
...
- DGM → goes to `dgm-dash` (DGM Visit Dashboard)
- Admin → goes to `adm-compliance` (Compliance Dashboard)
- Regional Controller → goes to `adm-compliance` (Compliance Dashboard)
...
### SCREEN 13: DGM Visit Dashboard (`DGMDash.tsx`)

**Who uses it:** DGMs
**Purpose:** Shows scheduled and completed monthly oversight visits with action buttons — styled the same as the Controller dashboard (no coverage matrix)

**KPI Cards:**
- Scheduled (upcoming visits)
- Completed (this month)
- Missed
- Locations with no visit this month

**Filter Bar:**
- Status chips: All | Scheduled | Completed | Missed
- Location dropdown

**Visit Table columns:** Date | Location | Status | Observed Total | vs Imprest | Notes/Reason | Actions
- Scheduled rows show ✓ Complete and × Missed action buttons that expand inline forms
- If a location already has a completed visit this month AND still has a scheduled visit, the Notes cell shows an amber warning chip: "⚠ Already visited this month on [date]"

**Actions (inline expand):**
- Complete: Enter observed total + notes → mark as completed
- Missed: Enter reason → mark as missed

---

### SCREEN 13b: DGM Visit History (`DGMHistory.tsx`)

**Who uses it:** DGMs
**Purpose:** Read-only history of all past DGM visits with rich filtering (accessible from "History" sidebar nav item)

**KPI Cards:** Total Visits | Completed | Scheduled | Missed

**Filters (5 dropdowns):** Location | Year | Month | Status | Variance Band
...
### SCREEN 15: Compliance Dashboard (`AdmCompliance.tsx`)

**Who uses it:** Regional Controller (primary), Admin
...
- Per location shows:
  ...
  - Today's submission status + total + variance
  - 30-day submission rate %
  - Last controller visit + days ago + DOW flag + next visit
  - DGM this month status + observed total

**Note:** The "Action Required" alert panel has been removed. Issues are visible directly in the location status table.
...
**Who uses it:** Admins
**Purpose:** Full CRUD management of cashroom locations, plus global defaults

**Table shows:** Code | Name | City | Expected Cash | Tolerance % | Status | Actions

**Add / Edit / Deactivate / Reactivate:** Same inline row pattern as before.

**Validation:** Name required, City required, Expected Cash must be positive, Tolerance 1–20%

**Global Defaults card (below table):**
- Default Tolerance % — applied to new locations unless overridden per-location
- Approval SLA (hours) — global SLA before submission is flagged as overdue
- Save Defaults button with 3-second success flash
...
**Who uses it:** Admins
**Purpose:** Full CRUD management of user accounts + system settings + screen access delegation

**Table shows:** Name | Email | Role | Locations | Status | Actions

**Add / Edit / Deactivate / Reactivate:** Same inline row pattern.

**Roles available:** Operator, Manager, Controller, DGM, Admin, Auditor, Regional Controller

**System Settings card (below user table):**
- DOW Lookback Window — 4-week or 6-week toggle (how far back to check day-of-week visit patterns)
- Daily Reminder Time — time picker for automated reminder sends
- Data Retention — number input 1–7 years
- Save Settings button

**Screen Access Delegation card (below System Settings):**

This is a key admin power feature. Admin can grant DGM and Regional Controller users temporary access to the Operator screen or Controller screen, independently.

Table columns: User | Role | 🏧 Operator Access | 🔍 Controller Access | Actions

Per access type per user:
- If not granted: shows NONE badge + "Grant" button
- If granted: shows GRANTED badge + grant date + note + "Edit" and "Revoke" buttons
- Clicking Grant or Edit opens a colour-coded inline form (blue for Operator, purple for Controller) with an optional Reason/Note field
- Revoke removes access immediately
- Grants persist in `localStorage` (`compass_operator_grants`, `compass_controller_grants`)

**Effect on DGM/RC users after access is granted:**
- A new nav item appears in their sidebar: "Operator View" (🏧) and/or "Controller View" (🔍)
- Their profile card in the sidebar shows 🏧 OPERATOR ACCESS and/or 🔍 CONTROLLER ACCESS badges
- They can use the full operator or controller flow for their assigned locations
- Access survives page refresh (localStorage) until admin explicitly revokes it

---

### SCREEN 18: Admin — Import Roster (`AdmImport.tsx`)

**Who uses it:** Admins
**Purpose:** Upload the Cashroom roster Excel to bulk-import users and location assignments

**Note:** The standalone Configuration screen (`AdmConfig.tsx`) has been removed. Global Defaults (Tolerance %, Approval SLA) are now managed via the Global Defaults card in Locations (Screen 16). System Settings (DOW Lookback, Daily Reminder Time, Data Retention) are now managed via the System Settings card in Users (Screen 17).

**Upload Zone:**
- Drag & drop or click to browse
- Accepts .xlsx, .xls, .csv
- Expected columns: CC #, District, Cashroom Lead, Manager/Daily Reviewer, Controller, DGM/RD, Regional Controller, Division Contacts

**Parsing:**
- Uses SheetJS (xlsx) client-side — no server round-trip for parsing
- Auto-detects the header row (finds row containing "CC")
- Handles continuation rows where CC # is blank (inherits from previous row)
- Empty rows skipped

**Preview table:** Shows all parsed rows with role-ownership badges colour-coded by column:
- CC # / District → LOCATION (blue)
- Cashroom Lead → OPERATOR (amber)
- Manager/Daily Reviewer → MANAGER (orange)
- Controller → CONTROLLER (purple)
- DGM/RD → DGM (green)
- Regional Controller / Division Contacts → REGIONAL CONTROLLER (sky blue)

**Summary KPIs:** Unique Locations | Unique Cashroom Leads | Unique Managers | Unique Controllers | Unique DGMs

**Confirm Import button:** Appears once rows are parsed. On click shows a success banner.

**Backend integration note:** When connected to the backend, the "Confirm Import" button should POST the parsed rows to a bulk-import endpoint that creates/updates user accounts and location assignments.
...
### SCREEN 21: Regional Controller — Cash Count Trends (`RcTrends.tsx`)

**Who uses it:** Regional Controller
**Purpose:** Interactive area chart showing average cash count section totals over time, with granularity and location filters

**Filters:**
- Granularity toggle: Weekly | Monthly | Quarterly
- Period selector: Last 8/12/24 weeks · Last 6/12 months · Last 4/8 quarters
- Location pills: All + one pill per active location (click to filter to a single location)

**Section tabs (A–I):** One tab per OpForm section. Clicking a tab switches the chart to show that section's trend. Only one section is shown at a time.

**KPI Summary row (above chart):**
- Latest period value (in section colour)
- % change vs previous period (green ▲ / red ▼)
- Period average
- Peak value in range

**Chart:** Area chart (Recharts AreaChart) with gradient fill in the active section's colour. Single metric over time — clean, focused trend view.

**Data:** Fully self-contained dummy data generated at module level using a deterministic seeded PRNG. Covers 2 years of history. Includes:
- Seasonal patterns (summer dip, December peak)
- Gentle upward trend (~0.4%/month)
- Per-location volume multipliers (each location has a slightly different cash base)
- Weekly data has higher variance than monthly, quarterly is smoothest

**No backend dependency** — this screen generates its own synthetic data. When real data is available, replace the generator functions (`monthlyPts`, `weeklyPts`, `quarterlyPts`) with API calls to a `/v1/reports/section-trends` endpoint.
...
### Step 6b: Screen Access Delegation API

The frontend currently stores screen access grants in `localStorage`. For production, persist these server-side:

```
GET    /v1/admin/access-grants              List all grants
POST   /v1/admin/access-grants              Create a grant
PUT    /v1/admin/access-grants/{id}         Update note
DELETE /v1/admin/access-grants/{id}         Revoke grant
```
...
2. Target user must have role DGM or REGIONAL_CONTROLLER → 422 if not
3. access_type must be "operator" or "controller"
4. Upsert: if grant already exists for this user+type, update note and grantedAt
5. Write ACCESS_GRANTED audit event

Output: { id, user_id, access_type, note, granted_at, granted_by }
```

**Frontend integration (`AdmUsers.tsx`):**
Replace the `readGrants()` / `writeGrants()` localStorage calls with API calls to these endpoints.

**Frontend integration (`App.tsx`):**
Replace `hasAccess(userId, type)` localStorage check with the result of `GET /v1/auth/me` — include `access_grants: ["operator"|"controller"]` in the me response so the frontend knows what nav items to show without a separate call.
...
-- ENUMS
-- ============================================================
CREATE TYPE user_role AS ENUM ('OPERATOR', 'MANAGER', 'CONTROLLER', 'DGM', 'ADMIN', 'AUDITOR', 'REGIONAL_CONTROLLER');
...
CREATE TABLE access_grants (
    ...
    access_type     VARCHAR(20) NOT NULL CHECK (access_type IN ('operator', 'controller')),
    note            TEXT,
...
| GET | /v1/auth/me | Authenticated | Get current user info including active access grants |
...
| DELETE | /v1/admin/access-grants/{id} | Admin | Revoke access grant |
...
| TC-004 | Valid login — DGM | Enter dgm@compass.com / demo1234 | Redirected to DGM Visit Dashboard. |
| TC-005 | Valid login — Admin | Enter admin@compass.com / demo1234 | Redirected to Compliance Dashboard. Sidebar shows all admin nav items. |
| TC-005b | Valid login — Regional Controller | Enter regionalcontroller@compass.com / demo1234 | Redirected to Compliance Dashboard. Sidebar shows RC nav items. |
...
### TC-143 to TC-158: DGM Visit Dashboard (DGMDash) and DGM Visit History (DGMHistory)

| ID | Test Case | Steps | Expected Result |
|---|---|---|---|
| TC-143 | Dashboard loads with KPI cards | Navigate to DGMDash | KPI cards: Scheduled, Completed, Missed, Locations with no visit this month |
| TC-144 | Completed visit row shows green status | View a completed visit row | Status badge shows "Completed" in green |
| TC-145 | Scheduled visit row shows action buttons | View a scheduled visit row | ✓ Complete and × Missed buttons visible on that row |
| TC-146 | Missed visit row shows red status | View a missed visit | Status badge shows "Missed" in red |
| TC-147 | Filter by Status — Scheduled | Click "Scheduled" chip | Only scheduled visits shown |
| TC-148 | Complete action — inline form | Click ✓ Complete on a scheduled row | Inline form expands asking for observed total + notes |
| TC-149 | Missed action — inline form | Click × Missed on a scheduled row | Inline form expands asking for reason |
| TC-150 | Already visited warning chip | Location has completed visit AND scheduled visit in same month | Notes cell shows amber chip: "⚠ Already visited this month on [date]" |
| TC-151 | DGM History — loads | Navigate to DGMHistory | Table shows all past DGM visits |
| TC-152 | DGM History — KPI cards | View KPI row | Total Visits, Completed, Scheduled, Missed counts shown |
| TC-153 | DGM History — filter by location | Select a location from dropdown | Only rows for that location shown |
| TC-154 | DGM History — filter by year | Select a year | Only visits from that year shown |
| TC-155 | DGM History — filter by month | Select March | Only March visits shown |
| TC-156 | DGM History — filter by variance band | Select "Over tolerance" | Only visits where DGM observed variance > tolerance |
| TC-157 | DGM History — clear all filters | Click "Clear all" | All history rows shown again |
...
| TC-171 | Non-compliant location shown in table | When any location has an issue | That location row shows red health badge (✕ Non-Compliant) in the location status table |
| TC-172 | At-risk location shown in table | Location with advisory-level issue | Row shows amber health badge (⚠ At Risk) |
| TC-173 | Issues visible inline | Review location table rows | Submission status, controller visit, DGM status visible per row without a separate panel |
| TC-174 | Most Critical sort | Default sort order | Non-Compliant (red) rows appear first, then At Risk, then Compliant |
...
### TC-211 to TC-220: Admin — Global Defaults & System Settings (formerly AdmConfig — now split across Locations and Users screens)

**Note:** The standalone Configuration screen (`AdmConfig.tsx`) has been removed. Global Defaults (Tolerance %, Approval SLA) are now in the Locations screen (Screen 16). System Settings (DOW Lookback, Daily Reminder Time, Data Retention) are now in the Users screen (Screen 17).

| ID | Test Case | Steps | Expected Result |
|---|---|---|---|
| TC-211 | Global Defaults card loads | Navigate to Locations screen | Global Defaults card shows current Default Tolerance % and Approval SLA values |
| TC-212 | Change default tolerance | Change Default Tolerance % to 8, click Save Defaults | Confirmation flash shown for 3 seconds. Reloading shows 8. |
| TC-213 | Tolerance out of range | Enter 25 in Default Tolerance % field | Error: "Tolerance must be 1–20%" |
| TC-214 | SLA must be positive | Enter 0 in Approval SLA field | Error: "SLA must be at least 1 hour" |
| TC-215 | DOW lookback — 4 weeks | Navigate to Users → System Settings, click "4 weeks" | Button highlights. Saves as 4. |
| TC-216 | DOW lookback — 6 weeks | Click "6 weeks" button | Button highlights. Saves as 6. |
| TC-217 | Daily Reminder Time | Change time picker value, click Save Settings | New time saved. |
| TC-218 | Data Retention | Change data retention to 5 years, click Save Settings | Saved. Shows 5 on reload. |
| TC-219 | Save confirmation auto-dismisses | Click Save Defaults or Save Settings | "Changes saved." message appears and disappears after 3 seconds |
...
| ECT-006 | DGM visit on 31st — last day of month | DGM books visit for 31 Jan, monthYear='2026-01' | Visit created. Blocks second booking for Jan. Shows as January visit in DGM history. |
...
| ID | Scenario | Steps | Expected Result |
|---|---|---|---|
| ECT-107 | All locations compliant | All locations have submitted + approved today, controller + DGM up to date | Overall Compliance = 100%. All rows in location table show "✓ Compliant" in green. |
