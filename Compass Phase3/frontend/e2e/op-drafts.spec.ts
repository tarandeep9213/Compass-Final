...
const OPERATOR_EMAIL = 'ld@compass-usa.com'
...
test('DRAFT-003: discarding a draft removes it from My Drafts list', async ({ page, request }) => {
  ...
  // Either the draft count decreased, or "No drafts" is shown
  const noDraftsMsg = page.getByText(/No drafts/i)
  const remainingResume = page.getByRole('button', { name: /Resume/i })
  const countAfter = await remainingResume.count()

  const noDraftsVisible = await noDraftsMsg.isVisible({ timeout: 2000 }).catch(() => false)
  const countDecreased = countAfter < countBefore

  expect(noDraftsVisible || countDecreased).toBe(true)
})
