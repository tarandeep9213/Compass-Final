 ...
 *   - ld@compass-usa.com  → assigned to at least one location
 *   - terri.serrano@compass.com → assigned to the SAME location
...
test('WORKFLOW-EXCEL-001: operator Excel submission appears as pending in controller Daily Review Dashboard', async ({ page, request }) => {
  ...
  const opToken = await getToken(request, 'ld@compass-usa.com', 'demo1234')
  if (!opToken) {
    test.skip(true, 'ld@compass-usa.com not found in DB — seed data missing')
    return
  }
  const ctrlToken = await getToken(request, 'terri.serrano@compass.com', 'demo1234')
  if (!ctrlToken) {
    test.skip(true, 'terri.serrano@compass.com not found in DB — seed data missing')
    return
  }
