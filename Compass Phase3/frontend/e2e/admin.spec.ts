...
test('ADMIN-001: admin creates a new location', async ({ page }) => {
  ...
  const nameIdx = await idInput.isVisible({ timeout: 2000 }).catch(() => false) ? 1 : 0
  ...
  const inTable = await tableBody.getByText(testLocName).isVisible({ timeout: 5000 }).catch(() => false)
