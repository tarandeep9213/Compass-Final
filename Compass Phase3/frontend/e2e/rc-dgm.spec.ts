...
test('RC-031: audit trail table has all column headers', async ({ page }) => {
  ...
  if (hasTable) {
    ...
    // kyle.decker@compass.com has 0 audit events in demo data — empty state is acceptable
...
test('DGM-019: DGM lands on Coverage Dashboard after login', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: /Daily Review Dashboard/i })).not.toBeVisible()
})
