...
class TestListBuildings:
    ...
    def test_admin_can_list(self, client, admin_token):
        r = client.get("/v1/alarm/buildings", headers=_headers(admin_token))
        assert r.status_code == 200
        body = r.json()
...
class TestCreateBuilding:
    ...
    def test_create_returns_in_list(self, client, admin_token):
        # Create a building
        r = client.post("/v1/alarm/buildings",
            headers=_headers(admin_token),
    ...
    def test_create_with_exempt_status(self, client, admin_token):
        r = client.post("/v1/alarm/buildings",
            headers=_headers(admin_token),
            json={
                **SAMPLE_BUILDING,
    ...
    def test_create_missing_name_fails(self, client, admin_token):
        r = client.post("/v1/alarm/buildings",
            headers=_headers(admin_token),
            json={"region": "Midwest"},
        )
...
class TestGetBuilding:
    ...
    def test_get_existing(self, client, admin_token):
        ...
        # Get it
        r = client.get(f"/v1/alarm/buildings/{bid}", headers=_headers(admin_token))
        assert r.status_code == 200
        assert r.json()["name"] == "Get Test Building"

    def test_get_nonexistent_returns_404(self, client, admin_token):
        r = client.get("/v1/alarm/buildings/nonexistent-id", headers=_headers(admin_token))
...
class TestUpdateBuilding:
    ...
    def test_update_status_to_closed(self, client, admin_token):
        r = client.post("/v1/alarm/buildings",
            headers=_headers(admin_token),
    ...
    def test_update_assigned_testers(self, client, admin_token):
        r = client.post("/v1/alarm/buildings",
            headers=_headers(admin_token),
...
class TestResetBuildings:
    ...
    def test_reset_deletes_all(self, client, admin_token):
        ...
        # Reset
        r = client.post("/v1/alarm/buildings/reset", headers=_headers(admin_token))
        assert r.status_code == 200
