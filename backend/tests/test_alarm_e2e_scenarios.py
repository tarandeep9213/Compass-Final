...
def _biannual_approved(client, ctrl_token, alarm_approver_token, bid, check_type, check_date, status="COMPLIANT", **kwargs):
    ...
    client.post(f"/v1/alarm/biannual/{cid}/approve", headers=_h(alarm_approver_token), json={"notes": "OK"})
    return cid
...
class TestScenario_NewBuildingOnboarding:
    ...
    def test_full_onboarding(self, client, alarm_admin_token, alarm_approver_token, alarm_tester_token):
        # ── Screen 13: Building Setup ──
        r = client.post("/v1/alarm/buildings", headers=_h(alarm_admin_token), json={
        ...
        r = client.post("/v1/alarm/zones/import", headers=_h(alarm_admin_token),
        ...
        r = client.post("/v1/alarm/access", headers=_h(alarm_admin_token), json={
        ...
        r = client.get("/v1/alarm/rules", headers=_h(alarm_admin_token))
...
class TestScenario_TesterMonthlyWorkflow:
    ...
    def test_monthly_test_flow(self, client, alarm_admin_token, alarm_approver_token, alarm_tester_token):
        today = date.today()
        month = f"{today.year}-{today.month:02d}"

        # Setup: building + zones
        r = client.post("/v1/alarm/buildings", headers=_h(alarm_admin_token),
        ...
        for i, (name, ztype) in enumerate([
            ...
            r = client.post("/v1/alarm/zones", headers=_h(alarm_admin_token),
        ...
        r = client.post("/v1/alarm/tests", headers=_h(alarm_tester_token), json={
            "building_id": bid,
        ...
        assert r.json()["tester_name"] == "Tara Tester"
        ...
        r = client.post(f"/v1/alarm/tests/{tid}/zones", headers=_h(alarm_tester_token),
        ...
        r = client.post(f"/v1/alarm/tests/{tid}/attachments", headers=_h(alarm_tester_token),
        ...
        r = client.post(f"/v1/alarm/tests/{tid}/submit", headers=_h(alarm_tester_token))
        assert r.status_code == 200
        ...
        r = client.get("/v1/alarm/tests?status=SUBMITTED", headers=_h(alarm_admin_token))
        ...
        r = client.post(f"/v1/alarm/tests/{tid}/approve", headers=_h(alarm_approver_token),
...
class TestScenario_RejectionFixResubmit:
    ...
    def test_double_rejection_then_approve(self, client, alarm_admin_token, alarm_approver_token, alarm_tester_token):
        today = date.today()
        month = f"{today.year}-{today.month:02d}"
        ...
        bid = r.json()["id"]
        z1 = client.post("/v1/alarm/zones", headers=_h(alarm_admin_token),
            json={"building_id": bid, "zone_number": 1, "zone_name": "Front Door", "zone_type": "ENTRY_EXIT", "area_number": 1}).json()["id"]
        z2 = client.post("/v1/alarm/zones", headers=_h(alarm_admin_token),
        ...
        tid = client.post("/v1/alarm/tests", headers=_h(alarm_tester_token),
        ...
        client.post(f"/v1/alarm/tests/{tid}/submit", headers=_h(alarm_tester_token))

        # Admin rejects: "Zone 2 not tested"
        r = client.post(f"/v1/alarm/tests/{tid}/reject", headers=_h(alarm_approver_token),
        ...
        # Verify zone update
        detail = client.get(f"/v1/alarm/tests/{tid}", headers=_h(alarm_tester_token)).json()
        assert detail["test"]["zones_issue"] == 1

        # Admin rejects again: "Issue not resolved"
        client.post(f"/v1/alarm/tests/{tid}/reject", headers=_h(alarm_approver_token),
...
class TestScenario_BiannualCycle:
    ...
    def test_biannual_lifecycle(self, client, alarm_admin_token, alarm_approver_token, alarm_tester_token):
        r = client.post("/v1/alarm/buildings", headers=_h(alarm_admin_token),
        ...
        _biannual_approved(client, alarm_tester_token, alarm_approver_token, bid, "CELLULAR_BACKUP", "2026-01-15")
        ...
        _biannual_approved(client, alarm_tester_token, alarm_approver_token, bid, "CAMERA_BACKUP", "2026-02-01", status="NON_COMPLIANT", days_verified=20)
        ...
        _biannual_approved(client, alarm_tester_token, alarm_approver_token, bid, "CAMERA_BACKUP", "2026-03-01", days_verified=32)
        ...
        r = client.get(f"/v1/alarm/biannual?building_id={bid}", headers=_h(alarm_tester_token))
...
class TestScenario_EscalationWorkflow:
    ...
    def test_escalation_to_resolution(self, client, alarm_admin_token, alarm_approver_token, alarm_tester_token):
        today = date.today()
        month = f"{today.year}-{today.month:02d}"
        ...
        r = client.post("/v1/alarm/escalation/remind", headers=_h(alarm_admin_token),
        ...
        client.post(f"/v1/alarm/tests/{tid}/approve", headers=_h(alarm_approver_token),
...
class TestScenario_MultiRegionReview:
    ...
    def test_multi_region_dashboard(self, client, alarm_admin_token, alarm_approver_token, alarm_tester_token):
        today = date.today()
        month = f"{today.year}-{today.month:02d}"

        # Create buildings in 3 regions
        b_mw = client.post("/v1/alarm/buildings", headers=_h(alarm_admin_token),
            json={"name": "MW Compliant", "region": "Midwest"}).json()["id"]
        b_se = client.post("/v1/alarm/buildings", headers=_h(alarm_admin_token),
            json={"name": "SE Overdue", "region": "Southeast"}).json()["id"]
        b_ne = client.post("/v1/alarm/buildings", headers=_h(alarm_admin_token),
        ...
        client.post(f"/v1/alarm/tests/{tid}/submit", headers=_h(alarm_tester_token))
        client.post(f"/v1/alarm/tests/{tid}/approve", headers=_h(alarm_approver_token), json={"notes": "OK"})

        # NE: submitted (pending)
        tid2 = client.post("/v1/alarm/tests", headers=_h(alarm_tester_token),
            json={"building_id": b_ne, "test_date": today.isoformat(), "test_month": month}).json()["id"]
        client.post(f"/v1/alarm/tests/{tid2}/submit", headers=_h(alarm_tester_token))
        ...
        r = client.get("/v1/alarm/dashboard/overview?region=Midwest", headers=_h(alarm_admin_token))
...
class TestScenario_BuildingLifecycle:
    ...
    def test_building_status_journey(self, client, alarm_admin_token, alarm_approver_token, alarm_tester_token):
        today = date.today()
        month = f"{today.year}-{today.month:02d}"
...
class TestScenario_TestHistoryFiltering:
    ...
    def test_history_filters(self, client, alarm_admin_token, alarm_approver_token, alarm_tester_token):
        today = date.today()
        month = f"{today.year}-{today.month:02d}"

        # Setup 2 buildings
        b1 = client.post("/v1/alarm/buildings", headers=_h(alarm_admin_token),
            json={"name": "History Bld 1", "region": "Midwest"}).json()["id"]
        b2 = client.post("/v1/alarm/buildings", headers=_h(alarm_admin_token),
            json={"name": "History Bld 2", "region": "Southeast"}).json()["id"]

        # Create tests in different states
        t1 = client.post("/v1/alarm/tests", headers=_h(alarm_tester_token),
            json={"building_id": b1, "test_date": today.isoformat(), "test_month": month}).json()["id"]
        # t1 stays DRAFT

        t2 = client.post("/v1/alarm/tests", headers=_h(alarm_tester_token),
            json={"building_id": b2, "test_date": today.isoformat(), "test_month": month}).json()["id"]
        client.post(f"/v1/alarm/tests/{t2}/submit", headers=_h(alarm_tester_token))
        # t2 is SUBMITTED

        t3 = client.post("/v1/alarm/tests", headers=_h(alarm_tester_token),
            json={"building_id": b1, "test_date": (today - timedelta(days=1)).isoformat(), "test_month": month}).json()["id"]
        client.post(f"/v1/alarm/tests/{t3}/submit", headers=_h(alarm_tester_token))
        client.post(f"/v1/alarm/tests/{t3}/approve", headers=_h(alarm_approver_token), json={"notes": "OK"})
        # t3 is APPROVED

        # ── Screen 3: Filter by building ──
        r = client.get(f"/v1/alarm/tests?building_id={b1}", headers=_h(alarm_tester_token))
        bids = {t["building_id"] for t in r.json()}
        assert bids == {b1}

        # ── Screen 3: Filter by status DRAFT ──
        r = client.get("/v1/alarm/tests?status=DRAFT", headers=_h(alarm_tester_token))
        assert all(t["status"] == "DRAFT" for t in r.json())
        assert t1 in [t["id"] for t in r.json()]

        # ── Screen 3: Filter by status SUBMITTED ──
        r = client.get("/v1/alarm/tests?status=SUBMITTED", headers=_h(alarm_tester_token))
        assert all(t["status"] == "SUBMITTED" for t in r.json())

        # ── Screen 3: Filter by month ──
        r = client.get(f"/v1/alarm/tests?month={month}", headers=_h(alarm_tester_token))
        assert all(t["test_month"] == month for t in r.json())

        # ── Screen 3: Combined filters ──
        r = client.get(f"/v1/alarm/tests?building_id={b1}&status=APPROVED", headers=_h(alarm_tester_token))
        for t in r.json():
