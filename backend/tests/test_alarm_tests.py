...
def setup_building_and_zones(client):
    ...
    admin_token = _auth(client, "admin@compass.com")
    ...
    return {"building_id": bid, "zone_ids": zone_ids, "admin_token": admin_token}
...
class TestCreateAlarmTest:
    ...
    def test_create_draft(self, client, controller_token, setup_building_and_zones):
...
class TestListAlarmTests:
    ...
    def test_list_all(self, client, controller_token, setup_building_and_zones):
        r = client.get("/v1/alarm/tests", headers=_headers(controller_token))
        assert r.status_code == 200
        assert isinstance(r.json(), list)

    def test_filter_by_building(self, client, controller_token, setup_building_and_zones):
        bid = setup_building_and_zones["building_id"]
        r = client.get(f"/v1/alarm/tests?building_id={bid}", headers=_headers(controller_token))
        assert r.status_code == 200
        for t in r.json():
            assert t["building_id"] == bid

    def test_filter_by_status(self, client, controller_token):
        r = client.get("/v1/alarm/tests?status=DRAFT", headers=_headers(controller_token))
        assert r.status_code == 200
        for t in r.json():
            assert t["status"] == "DRAFT"

    def test_filter_by_month(self, client, controller_token):
        r = client.get("/v1/alarm/tests?month=2026-04", headers=_headers(controller_token))
        assert r.status_code == 200
        for t in r.json():
...
class TestGetAlarmTest:
    ...
    def test_get_with_zones_and_attachments(self, client, controller_token, setup_building_and_zones):
        bid = setup_building_and_zones["building_id"]
        # Create a test
        r = client.post("/v1/alarm/tests",
            headers=_headers(controller_token),
    ...
    def test_get_nonexistent_returns_404(self, client, controller_token):
        r = client.get("/v1/alarm/tests/nonexistent", headers=_headers(controller_token))
...
class TestSaveZoneResults:
    ...
    def test_save_zone_results(self, client, controller_token, setup_building_and_zones):
        bid = setup_building_and_zones["building_id"]
...
class TestSubmitAlarmTest:
    ...
    def test_submit_draft(self, client, controller_token, setup_building_and_zones):
        bid = setup_building_and_zones["building_id"]
        r = client.post("/v1/alarm/tests",
            headers=_headers(controller_token),
        ...
        r = client.post(f"/v1/alarm/tests/{tid}/submit", headers=_headers(controller_token))
        assert r.status_code == 200
        assert r.json()["status"] == "SUBMITTED"
        assert r.json()["submitted_at"] is not None

    def test_cannot_submit_already_submitted(self, client, controller_token, setup_building_and_zones):
        bid = setup_building_and_zones["building_id"]
        r = client.post("/v1/alarm/tests",
            headers=_headers(controller_token),
...
class TestApproveAlarmTest:
    ...
    def test_admin_can_approve(self, client, controller_token, setup_building_and_zones):
        admin_token = setup_building_and_zones["admin_token"]
        bid = setup_building_and_zones["building_id"]
    ...
    def test_controller_cannot_approve(self, client, controller_token, setup_building_and_zones):
        bid = setup_building_and_zones["building_id"]
        r = client.post("/v1/alarm/tests",
            headers=_headers(controller_token),
    ...
    def test_cannot_approve_draft(self, client, setup_building_and_zones):
        admin_token = setup_building_and_zones["admin_token"]
        bid = setup_building_and_zones["building_id"]
...
class TestRejectAlarmTest:
    ...
    def test_admin_can_reject_with_reason(self, client, controller_token, setup_building_and_zones):
        admin_token = setup_building_and_zones["admin_token"]
        bid = setup_building_and_zones["building_id"]
    ...
    def test_reject_requires_reason(self, client, setup_building_and_zones):
        admin_token = setup_building_and_zones["admin_token"]
        bid = setup_building_and_zones["building_id"]
        ...
        tid = r.json()["id"]
        client.post(f"/v1/alarm/tests/{tid}/submit", headers=_headers(admin_token))
...
class TestReopenAlarmTest:
    ...
    def test_reopen_rejected_to_draft(self, client, controller_token, setup_building_and_zones):
        admin_token = setup_building_and_zones["admin_token"]
        bid = setup_building_and_zones["building_id"]
        ...
        # Reopen
        r = client.post(f"/v1/alarm/tests/{tid}/reopen", headers=_headers(controller_token))
        assert r.status_code == 200
        assert r.json()["status"] == "DRAFT"
        assert r.json()["rejection_reason"] is None

    def test_cannot_reopen_non_rejected(self, client, controller_token, setup_building_and_zones):
        bid = setup_building_and_zones["building_id"]
        r = client.post("/v1/alarm/tests",
            headers=_headers(controller_token),
...
class TestFullLifecycle:
    ...
    def test_complete_lifecycle(self, client, controller_token, setup_building_and_zones):
        admin_token = setup_building_and_zones["admin_token"]
        bid = setup_building_and_zones["building_id"]
        zids = setup_building_and_zones["zone_ids"]
        ...
        client.post(f"/v1/alarm/tests/{tid}/zones",
            headers=_headers(controller_token), json={"results": results})
        ...
        # 7. Approve
        r = client.post(f"/v1/alarm/tests/{tid}/approve",
            headers=_headers(admin_token), json={"notes": "All good now"})
        assert r.json()["status"] == "APPROVED"
