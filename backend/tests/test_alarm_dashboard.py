...
def _create_and_approve_test(client, ctrl_token: str, approver_token: str, bid: str, month: str):
    ...
    client.post(f"/v1/alarm/tests/{tid}/approve",
        headers=_headers(approver_token), json={"notes": "OK"})
    return tid
...
class TestAlarmOverview:
    ...
    def test_returns_summary_and_buildings(self, client, alarm_admin_token):
        r = client.get("/v1/alarm/dashboard/overview", headers=_headers(alarm_admin_token))
        assert r.status_code == 200
        body = r.json()
    ...
    def test_building_rows_have_status(self, client, alarm_admin_token):
        _create_building(client, alarm_admin_token, "Dash Active")
        _create_building(client, alarm_admin_token, "Dash Exempt", "temporarily_exempt")
    ...
    def test_compliant_building_counted(self, client, alarm_admin_token, alarm_tester_token, alarm_approver_token):
        bid = _create_building(client, alarm_admin_token, "Dash Compliant")
        today = date.today()
        month = f"{today.year}-{today.month:02d}"
        _create_and_approve_test(client, alarm_tester_token, alarm_approver_token, bid, month)
    ...
    def test_monthly_history_has_12_months(self, client, alarm_admin_token):
        r = client.get("/v1/alarm/dashboard/overview", headers=_headers(alarm_admin_token))
        assert len(r.json()["monthly_history"]) == 12

    def test_filter_by_region(self, client, alarm_admin_token):
        r = client.get("/v1/alarm/dashboard/overview?region=Midwest", headers=_headers(alarm_admin_token))
        assert r.status_code == 200
...
class TestAlarmTrends:
    ...
    def test_returns_months_array(self, client, alarm_admin_token):
        r = client.get("/v1/alarm/dashboard/trends", headers=_headers(alarm_admin_token))
        assert r.status_code == 200
        body = r.json()
    ...
    def test_each_month_has_fields(self, client, alarm_admin_token):
        r = client.get("/v1/alarm/dashboard/trends", headers=_headers(alarm_admin_token))
    ...
    def test_custom_months_param(self, client, alarm_admin_token):
        r = client.get("/v1/alarm/dashboard/trends?months=6", headers=_headers(alarm_admin_token))
        assert r.status_code == 200
...
class TestBuildingDrillDown:
    ...
    def test_returns_building_detail(self, client, alarm_admin_token):
        bid = _create_building(client, alarm_admin_token, "DrillDown Building")

        r = client.get(f"/v1/alarm/dashboard/building/{bid}", headers=_headers(alarm_admin_token))
        assert r.status_code == 200
        body = r.json()
    ...
    def test_includes_zones(self, client, alarm_admin_token):
        bid = _create_building(client, alarm_admin_token, "DrillDown Zones")
        # Add a zone
    ...
    def test_includes_tests(self, client, alarm_admin_token, alarm_tester_token, alarm_approver_token):
        bid = _create_building(client, alarm_admin_token, "DrillDown Tests")
        today = date.today()
        month = f"{today.year}-{today.month:02d}"
        _create_and_approve_test(client, alarm_tester_token, alarm_approver_token, bid, month)
    ...
    def test_nonexistent_returns_404(self, client, alarm_admin_token):
        r = client.get("/v1/alarm/dashboard/building/nonexistent", headers=_headers(alarm_admin_token))
