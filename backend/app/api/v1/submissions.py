...
def update_draft(
    ...
    if s.operator_id != current_user.id:
        raise HTTPException(403, "Access denied")
    # Operators can edit a draft (never submitted), a rejected submission
    # (fix & resubmit), or a still-pending submission (catch mistakes before
    # the controller reviews). Once APPROVED the data is locked.
